...
const main = () => {
  ...
  try {
    ...
    if (!funcnames.length) {
      console.error('Action(s), seperated by space, are required');
      process.exit(1);
    }
    if (!name) {
      console.error('"--name=" argument required');
      process.exit(1);
    }

    const typenames = funcnames.map(
      (str) => `${name.toUpperCase()}_${snakeCaseCap(str)}`
    );
    ...
    if (a && n)
      console.log(`Created ${funcnames.length} action(s) for [${name}]`);
    if (r && n) console.log(`Created a reducer file for [${name}]`);
    if (s && n) console.log(`Created a saga file for [${name}]`);

    if (a && !n)
      console.log(`Added ${funcnames.length} action(s) for [${name}]`);
    if (r && !n)
      console.log(`Added ${funcnames.length} reducer(s) for [${name}]`);
